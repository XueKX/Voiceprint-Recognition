...
add_arg('threshold', float, 0.85, '判断是否为同一个人的阈值')
...
if __name__ == '__main__':
    t1 = time.time()
    # 要预测的两个人的音频文件
    audio_path_01 = "audio/other/carry_long_01-f.wav"  # 相似度为：0.9899199604988098
    audio_path_02 = "audio/other/carry_01.wav"

    # audio_path_01 = "audio/loupan/13057366090/black_8000.wav"
    # audio_path_02 = "audio/loupan/13057366090/grey_2000321.wav"
    feature1 = infer(audio_path_01)[0]
    feature2 = infer(audio_path_02)[0]
    # 对角余弦值
    dist = np.dot(feature1, feature2) / (np.linalg.norm(feature1) * np.linalg.norm(feature2))
    if dist > args.threshold:
        print(f"{audio_path_01} 和 {audio_path_02} 为同一个人，相似度为：{dist}")
    else:
        print(f"{audio_path_01} 和 {audio_path_02} 不是同一个人，相似度为：{dist}")
    print(f'Call time :{time.time() - t1}')
